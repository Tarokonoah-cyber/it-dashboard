...
export async function runOperationsReminder(today = todayTaipei()) {
  const [generation, followUps] = await Promise.all([
    generateRecurringTasks(today),
    promoteDueFollowUps(today)
  ]);
  const line = await runDailyLineDigest(today);
  return { today, generation, followUps, line };
}
