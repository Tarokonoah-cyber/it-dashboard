...
function requiredText(value, label, maxLength) {
  ...
  if (text.length > maxLength) throw new Error(`${label}不可超過 ${maxLength} 字`);
  return text;
}

function requiredMoney(value, label, { allowNegative = false } = {}) {
  const parsed = parseMoney(value);
  if (parsed.empty || parsed.error || parsed.value === null) throw new Error(`${label}不是有效金額`);
  if (!allowNegative && parsed.value < 0) throw new Error(`${label}不可為負數`);
  return parsed.value;
}

export function normalizeManualBudgetPayload(input) {
  const body = input && typeof input === "object" ? input : {};
  const budgetYear = Number(body.budgetYear);
  if (!Number.isInteger(budgetYear) || budgetYear < 2000 || budgetYear > 2200) {
    throw new Error("預算年度不正確");
  }

  const monthlyAmounts = new Map();
  for (const row of Array.isArray(body.monthlyAmounts) ? body.monthlyAmounts : []) {
    const actualYear = Number(row?.actualYear);
    const actualMonth = Number(row?.actualMonth);
    if (!Number.isInteger(actualYear) || actualYear < 2000 || actualYear > 2200) {
      throw new Error("動支年度不正確");
    }
    if (!Number.isInteger(actualMonth) || actualMonth < 1 || actualMonth > 12) {
      throw new Error("動支月份不正確");
    }
    const amount = requiredMoney(row?.amount, `${actualYear} 年 ${actualMonth} 月動支金額`, { allowNegative: true });
    const key = `${actualYear}-${actualMonth}`;
    monthlyAmounts.set(key, {
      actualYear,
      actualMonth,
      amount: roundCurrency((monthlyAmounts.get(key)?.amount || 0) + amount)
    });
  }

  return {
    budgetYear,
    budgetCode: requiredText(body.budgetCode, "預算編號", 120),
    itemName: requiredText(body.itemName, "預算項目", 500),
    department: requiredText(body.department, "部門名稱", 120),
    quantity: String(body.quantity || "").trim().slice(0, 100),
    budgetAmount: requiredMoney(body.budgetAmount, "預算金額"),
    committedAmount: body.committedAmount === "" || body.committedAmount === null || body.committedAmount === undefined
      ? 0
      : requiredMoney(body.committedAmount, "送簽中金額"),
    monthlyAmounts: [...monthlyAmounts.values()].sort((left, right) =>
      left.actualYear - right.actualYear || left.actualMonth - right.actualMonth
    )
  };
}
