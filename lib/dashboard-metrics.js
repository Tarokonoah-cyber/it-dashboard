...
export function isCancelledWork(work) {
  return CANCELLED_STATUSES.has(String(work?.status || "").trim());
}
...
export function getMonthCompletionMetrics(works, today) {
  ...
  const monthWorks = (Array.isArray(works) ? works : []).filter((work) => {
    ...
    return Boolean(month) && workDate.startsWith(month) && !isCancelledWork(work);
  });
