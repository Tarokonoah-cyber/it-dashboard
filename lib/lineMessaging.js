...
function config() {
  return {
    ...
    groupId: String(process.env.LINE_ALLOWED_GROUP_ID || "").trim()
  };
}
...
export function getLineConfiguration() {
  ...
  return {
    configured: Boolean(current.token && current.groupId),
    tokenConfigured: Boolean(current.token),
    groupConfigured: Boolean(current.groupId)
  };
}
...
export async function pushLineText(message) {
  ...
  if (!current.groupId) throw new Error("LINE_ALLOWED_GROUP_ID 尚未設定");
