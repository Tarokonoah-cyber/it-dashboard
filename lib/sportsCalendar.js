...
function nullableDisplayText(value, maxLength = MAX_TEXT_LENGTH) {
  const text = cleanText(value, maxLength);
  return isInvalidDisplayText(text) ? null : text;
}

function normalizePitcher(value) {
  if (value === undefined || value === null) return null;
  if (typeof value === "string") return nullableDisplayText(value, 160);
  if (typeof value !== "object" || Array.isArray(value)) {
    throw validationError("probable pitcher must be a string or object");
  }
  return {
    name: nullableDisplayText(value.name, 160),
    team: nullableDisplayText(value.team, 160),
    throws: nullableText(value.throws, 40),
    source_status: nullableDisplayText(value.source_status || value.status, 80)
  };
}

function normalizeBaseballDetails(details) {
  const input = cleanObject(details, "details");
  const startingPitchers = cleanObject(input.starting_pitchers, "starting_pitchers");
  const probablePitchers = cleanObject(input.probable_pitchers, "probable_pitchers");
  const score = cleanObject(input.score, "score");
  const finalScore = cleanObject(input.final_score || input.score, "final_score");
  const weather = cleanObject(input.weather, "weather");
  const sync = cleanObject(input.sync, "sync");
  const awayPitcher = normalizePitcher(startingPitchers.away || probablePitchers.away);
  const homePitcher = normalizePitcher(startingPitchers.home || probablePitchers.home);
  ...
  return {
    ...
    score: {
      away: awayScore,
      home: homeScore,
      status: nullableDisplayText(score.status || finalScore.status, 80)
    },
    probable_pitchers: {
      away: awayPitcher,
      home: homePitcher,
      status: nullableDisplayText(probablePitchers.status || input.probable_pitchers_status, 80)
    },
    weather: Object.keys(weather).length ? weather : null,
    final_score: Object.keys(finalScore).length ? {
      away: awayScore,
      home: homeScore,
      status: nullableDisplayText(finalScore.status, 80)
    } : null,
    sync: Object.keys(sync).length ? {
      provider: nullableDisplayText(sync.provider, 240),
      synced_at: nullableDisplayText(sync.synced_at, 120),
      reasons: Array.isArray(sync.reasons) ? sync.reasons.map((item) => cleanText(item, 120)).filter(Boolean) : [],
      limitations: Array.isArray(sync.limitations) ? sync.limitations.map((item) => cleanText(item, 240)).filter(Boolean) : []
    } : null,
