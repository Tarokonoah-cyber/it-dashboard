...
import { DASHBOARD_LOGIN_USER, verifyDashboardCredentials } from "./auth";
...
export async function verifyDashboardLoginCredentials(user, password) {
  if (String(user || "").trim() !== DASHBOARD_LOGIN_USER) return false;
  const stored = await getStoredDashboardCredentials();
  if (!stored) return verifyDashboardCredentials(user, password);
  if (String(stored.username || "").trim() !== DASHBOARD_LOGIN_USER) return false;
...
export async function updateDashboardPassword(user, nextPassword) {
  if (String(user || "").trim() !== DASHBOARD_LOGIN_USER) {
    throw new Error("Invalid dashboard user");
  }
