...
function normalizeSoftwareContractStatus(value) {
  const status = text(value);
  if (/中止|終止|取消|失效|停用|terminated|cancel|inactive/i.test(status)) return "中止";
  if (/即期|expiring/i.test(status)) return "即期";
  return "有效";
}
...
function mapSoftwareContract(values, id) {
  const mapped = {
    ...
    status: normalizeSoftwareContractStatus(pick(values, FIELD_KEYS.contracts_software.status)),
