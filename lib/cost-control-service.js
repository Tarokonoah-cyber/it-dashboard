...
export async function loadCostControlDashboard(filters) {
  ...
  try {
    imports = await supabaseRequest(
      "budget_imports",
      "select=id,original_filename,file_hash,budget_year,data_month,version_number,warning_count,confirmed_at,created_at,source_sheet_names,source_type&import_status=eq.succeeded&is_active=eq.true&order=confirmed_at.desc.nullslast,created_at.desc"
    );
  } catch (error) {
  ...
  return {
    ...
    meta: {
      ...
      sourceType: selectedImport.source_type || "excel"
    },
    options: {
      years,
...
export async function upsertManualBudgetItem(itemId, payload) {
  return supabaseRpc("upsert_manual_budget_item", {
    p_item_id: itemId || null,
    p_payload: payload,
    p_updated_by: dashboardLoginUser()
  });
}
