...
export async function searchDashboard(query, { includePasswords = true, excludeSources = [], totalLimit, categoryLimit } = {}) {
  const excluded = new Set((excludeSources || []).map(String));
  const sources = (includePasswords ? [...SOURCES, PASSWORD_SOURCE] : SOURCES)
    .filter((source) => !excluded.has(source.key));
