...
const WORK_LABEL = "\u5de5\u4f5c";
const SECRET_REPLY =
  "\u6211\u4e0d\u80fd\u63d0\u4f9b\u6216\u8655\u7406\u5bc6\u78bc\u3001API key\u3001env\u3001service role \u6216\u5176\u4ed6\u654f\u611f\u8cc7\u8a0a\u3002";
const DESTRUCTIVE_REPLY =
  "\u9019\u985e\u522a\u9664\u3001\u4fee\u6539\u3001\u66f4\u65b0\u6216\u6e05\u7a7a\u8cc7\u6599\u7684\u8acb\u6c42\u9700\u8981\u4eba\u5de5\u64cd\u4f5c\uff0cAI \u52a9\u7406\u4e0d\u6703\u76f4\u63a5\u57f7\u884c\u7834\u58de\u6027\u52d5\u4f5c\u3002";
...
const NAV_INTENTS = [
  ...
  { href: "/reports", label: `${OPEN_PREFIX}\u5831\u8868\u4e2d\u5fc3`, keywords: ["\u5831\u8868\u4e2d\u5fc3", "excel", "pdf", "reports"] }
];
...
function stripLeadingTodoWordsSafe(message) {
  return String(message || "")
    .trim()
    .replace(/^(please\s*)?(add|create|new)\s*(todo|task)?\s*[:：-]?\s*/i, "")
    .replace(/^(todo|task)\s*[:：-]?\s*/i, "")
    .replace(/^(請\s*)?(幫我\s*)?(新增|建立|加入|增加)\s*(一個|1個)?\s*(todo|待辦|工作|事項)?\s*[:：-]?\s*/u, "")
    .replace(/^(todo|待辦|工作|事項)\s*[:：-]?\s*/iu, "")
    .trim();
}

function inferTodoTitleSafe(message) {
  const title = stripLeadingTodoWordsSafe(message)
    .replace(/^(提醒我|請提醒我)\s*/u, "")
    .trim();
  if (!title || /^(todo|待辦|工作|事項)$/iu.test(title)) return "";
  return cleanText(title, MAX_TODO_TITLE_LENGTH);
}

function inferCalendarUnavailableSafe(message) {
  const raw = String(message || "").trim();
  const dateMatch = raw.match(/(今天|明天|後天|下週|下星期|本週|本星期|月底前|\d{1,2}[/-]\d{1,2}|\d{4}[/-]\d{1,2}[/-]\d{1,2})/u);
  const timeMatch = raw.match(/(上午|早上|中午|下午|晚上)?\s*(\d{1,2}|[一二三四五六七八九十兩])\s*(點|:|：)\s*\d{0,2}\s*(分)?/u);
  const title = cleanText(
    raw
      .replace(/^(請\s*)?(幫我\s*)?(新增|建立|加入|安排|排入|提醒我|提醒)\s*/u, "")
      .replace(/(行事曆|日曆|排程|提醒|會議|事件)/gu, "")
      .trim() || raw,
...
function inferCompleteWorkItemSafe(message) {
  const title = cleanText(
    String(message || "")
      .replace(/^(請\s*)?(幫我\s*)?(把|將)?\s*/u, "")
      .replace(/(標記|設為|改成)?\s*(已)?完成\s*$/u, "")
      .replace(/(工作|紀錄|工單)/gu, "")
      .trim(),
    MAX_TODO_TITLE_LENGTH
  );
...
function stripLeadingTodoWords(message) {
  return String(message || "")
    .trim()
    .replace(/^(please\s*)?(add|create|new)\s*(todo|task)?\s*[:：-]?\s*/i, "")
    .replace(/^(todo|task)\s*[:：-]?\s*/i, "")
...
function inferTodoTitle(message) {
  const title = stripLeadingTodoWords(message)
    .replace(/^(提醒我|請提醒我)\s*/u, "")
    .trim();
  if (!title || /^(todo|待辦|工作|事項)$/iu.test(title)) return "";
  return cleanText(title, MAX_TODO_TITLE_LENGTH);
...
function inferCalendarUnavailable(message) {
  const raw = String(message || "").trim();
  const dateMatch = raw.match(/(今天|明天|後天|下週|下星期|本週|本星期|\d{1,2}[/-]\d{1,2}|\d{4}[/-]\d{1,2}[/-]\d{1,2})/u);
  const timeMatch = raw.match(/(上午|早上|中午|下午|晚上)?\s*(\d{1,2}|[一二三四五六七八九十兩])\s*(點|:|：)\s*\d{0,2}\s*(分)?/u);
  const title = cleanText(
    raw
      .replace(/^(請\s*)?(幫我\s*)?(新增|建立|加入|安排|排入|提醒我|提醒)\s*/u, "")
...
function inferCompleteWorkItem(message) {
  const title = cleanText(
    String(message || "")
      .replace(/^(請\s*)?(幫我\s*)?(把|將)?\s*/u, "")
      .replace(/(標記|設為|改成)?\s*(已)?完成\s*$/u, "")
...
function createAnalysisAction(message) {
  ...
  if (normalized.includes("\u672a\u5b8c\u6210") || normalized.includes(TODO_LABEL) || normalized.includes("todo")) {
...
function wantsExplicitTodo(normalized) {
  return (
    normalized.includes("todo") ||
    normalized.includes(TODO_LABEL) ||
    normalized.includes("\u5efa\u7acb\u5f85\u8fa6") ||
    normalized.includes("\u52a0\u5165\u5f85\u8fa6")
  );
}

function wantsExplicitTodoSafe(normalized) {
  return (
    normalized.includes("todo") ||
    normalized.includes("\u5f85\u8fa6") ||
    normalized.includes("\u65b0\u589e\u5f85\u8fa6") ||
    normalized.includes("\u5efa\u7acb\u5f85\u8fa6") ||
    normalized.includes("\u52a0\u5165\u5f85\u8fa6")
  );
}

function hasQuestionTone(normalized) {
  return (
    normalized.includes("?") ||
    normalized.includes("\uff1f") ||
...
function looksLikeWorkTodo(message) {
  const raw = String(message || "").trim();
  const normalized = normalizeMessage(raw);
  if (!raw || raw.length > 60 || hasQuestionTone(normalized)) return false;
  if (hasAnyTerm(raw, SECRET_TERMS) || hasAnyTerm(raw, DESTRUCTIVE_TERMS) || hasAnyTerm(raw, SQL_OR_SCRIPT_TERMS)) return false;
  if (wantsCalendarSafe(normalized) || wantsCompleteWorkSafe(normalized)) return false;

  const hasWorkVerb =
    /檢查|確認|整理|追|處理|通知|更新|準備|維護|下單|勘查|盤點|測試|修|查|補|回覆|聯絡|安排/u.test(raw);
  const hasTimeHint = /今天|明天|後天|下午|上午|中午|晚上|月底前|本週|下週|週[一二三四五六日天]?|\d{1,2}[/-]\d{1,2}/u.test(raw);
  const hasWorkNoun = /ups|kpi|網路|電腦|投影機|機房|保險|電池|設備|系統|事務機|廠商|合約|sop/i.test(raw);

  return hasWorkVerb || (hasTimeHint && hasWorkNoun);
}

function wantsCalendar(normalized) {
  ...
  const hasReminderWithTime =
    normalized.includes("\u63d0\u9192") &&
    /(今天|明天|後天|下週|下星期|上午|早上|中午|下午|晚上|(\d{1,2}|[一二三四五六七八九十兩])\s*(點|:|：))/u.test(normalized);
  return hasCalendarTerm || hasReminderWithTime;
}

function wantsCalendarSafe(normalized) {
  const hasCalendarTerm =
    normalized.includes("\u884c\u4e8b\u66c6") ||
    normalized.includes("\u65e5\u66c6") ||
    normalized.includes("\u6392\u7a0b") ||
...
function wantsCompleteWork(normalized) {
  return (
    (normalized.includes("\u6a19\u8a18\u5b8c\u6210") ||
      normalized.includes("\u8a2d\u70ba\u5b8c\u6210") ||
      normalized.includes("\u6539\u6210\u5b8c\u6210") ||
      normalized.includes("mark complete")) &&
    (normalized.includes(WORK_LABEL) || normalized.includes("\u6aa2\u67e5") || normalized.includes("\u5de5\u55ae") || normalized.includes("\u7d00\u9304"))
  );
}

function wantsCompleteWorkSafe(normalized) {
  return (
    normalized.includes("\u6a19\u8a18\u5b8c\u6210") ||
    normalized.includes("\u8a2d\u70ba\u5b8c\u6210") ||
    normalized.includes("\u6539\u6210\u5b8c\u6210") ||
...
function wantsFollowUp(normalized) {
  return normalized.includes("新增待追蹤") || normalized.includes("建立待追蹤") || normalized.includes("加入待追蹤") || normalized.includes("追蹤事項");
}

function inferFollowUp(message) {
  const raw = String(message || "").trim();
  const dateMatch = raw.match(/(今天|明天|後天|下週|下星期|\d{1,2}[/-]\d{1,2}|\d{4}[/-]\d{1,2}[/-]\d{1,2})/u);
  const title = cleanText(raw.replace(/^(請\s*)?(幫我\s*)?(新增|建立|加入)?\s*(待追蹤|追蹤事項)\s*[:：-]?\s*/u, ""), MAX_TODO_TITLE_LENGTH);
  return { type: "create_follow_up", title, dateText: cleanText(dateMatch?.[0] || "", 60), note: "" };
}

function wantsBulkMutation(normalized) {
  return hasAnyTerm(normalized, BULK_MUTATION_TERMS) && wantsCompleteWorkSafe(normalized);
}

export function getUnsafeRequestReply(message) {
  if (hasAnyTerm(message, SECRET_TERMS)) return SECRET_REPLY;
  if (hasAnyTerm(message, DESTRUCTIVE_TERMS)) return DESTRUCTIVE_REPLY;
...
export function matchLocalIntent(message) {
  ...
  if (wantsBulkMutation(normalized)) {
    return { reply: DESTRUCTIVE_REPLY, action: null };
  }

  if (wantsCompleteWorkSafe(normalized)) {
    return {
      reply: "\u6211\u6703\u5148\u78ba\u8a8d\u662f\u5426\u53ea\u6709\u4e00\u7b46\u660e\u78ba\u5339\u914d\u7684\u5de5\u4f5c\u7d00\u9304\u3002",
      action: inferCompleteWorkItemSafe(message)
    };
  }
  ...
  if (wantsFollowUp(normalized)) {
    const action = inferFollowUp(message);
    return action.title ? { reply: "我會先整理待追蹤內容，確認後才會新增。", action } : { reply: "請提供明確的待追蹤事項。", action: null };
  }

  if (wantsExplicitTodoSafe(normalized) || looksLikeWorkTodo(message)) {
    const title = inferTodoTitleSafe(message);
    if (!title) {
      return {
  ...
  if (wantsCalendarSafe(normalized)) {
    return {
      reply: "\u76ee\u524d\u5100\u8868\u677f\u884c\u4e8b\u66c6\u5c1a\u672a\u63a5\u4e0a\u65b0\u589e\u529f\u80fd\u3002",
      action: inferCalendarUnavailableSafe(message)
    };
  }
...
export function sanitizeAssistantAction(action) {
  ...
  if (action.type === "create_follow_up") {
    ...
    return {
      ...
      title,
      dateText: cleanText(action.dateText || action.next_follow_date || action.date || "", 60),
      note: cleanText(action.note || action.description || "", MAX_ACTION_NOTE_LENGTH)
    };
  }

  if (action.type === "calendar_draft" || action.type === "calendar_unavailable" || action.type === "create_calendar_event") {
  ...
  if (action.type === "complete_work_item") {
    return {
      type: "complete_work_item",
      title: cleanText(action.title || action.query || "", MAX_TODO_TITLE_LENGTH)
    };
  }
...
export function sanitizeAssistantReply(reply) {
  ...
  if (!text) return "\u6211\u53ef\u4ee5\u5354\u52a9\u4f60\u5efa\u7acb Todo\u3001\u6574\u7406\u65e5\u66c6\u72c0\u614b\u3001\u7522\u751f\u5206\u6790\u6458\u8981\uff0c\u6216\u5e36\u4f60\u524d\u5f80 dashboard \u5167\u7684\u529f\u80fd\u9801\u9762\u3002";
  if (hasAnyTerm(text, SECRET_TERMS)) return SECRET_REPLY;
  if (hasAnyTerm(text, MUTATION_CLAIM_TERMS)) {
    return "\u6211\u76ee\u524d\u53ea\u6703\u57f7\u884c\u5df2\u5141\u8a31\u7684 Todo \u5efa\u7acb\u52d5\u4f5c\uff1b\u5176\u4ed6\u8cc7\u6599\u8b8a\u66f4\u9700\u8981\u5b89\u5168 API \u652f\u63f4\u3002";
  }
...
export function createFallbackReply(message) {
  ...
  return {
    reply: "\u6211\u76ee\u524d\u53ef\u4ee5\u5354\u52a9\u5efa\u7acb Todo\u3001\u8aaa\u660e dashboard \u884c\u4e8b\u66c6\u72c0\u614b\u3001\u6574\u7406\u672a\u5b8c\u6210\u6e05\u55ae\u3001\u5206\u6790\u5de5\u4f5c\u985e\u578b\u6216\u7522\u751f KPI \u5831\u544a\u8349\u7a3f\u3002\u8acb\u518d\u8f38\u5165\u66f4\u660e\u78ba\u7684\u4efb\u52d9\u3002",
    action: null
  };
}
