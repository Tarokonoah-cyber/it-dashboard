...
export async function fetchCpblEvents({ year = SEASON, lastSyncedAt = isoNow() } = {}) {
  ...
  if (!response.ok || !payload.Success) throw new Error(`CPBL schedule/getgamedatas failed HTTP ${response.status}`);
...
async function supabaseRequest(table, query, options = {}) {
  ...
  if (!response.ok) throw new Error(data.message || body || `Supabase ${table} HTTP ${response.status}`);
  return data;
}
